...
export default function NewInstitutionModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    let id_institution = "";
    try {
      const response: any = await api.post("/admin/institution", {
        name: formData.institutionName,
        code_state: formData.selectedUF,
        code_city: formData.selectedCity,
      });
  ...
  useEffect(() => {
    if (formData.selectedUF) {
      fetch(
        `https://servicodados.ibge.gov.br/api/v1/localidades/estados/${formData.selectedUF}/municipios`
      )
        .then((response) => {
          response.json().then((data) => {
  ...
  const canFinish = () => {
    return (
      (formData.coordinatorEmail.trim() != "" &&
        formData.coordinatorName.trim() !== "" &&
        formData.coordinatorPassword.trim() !== "" &&
        formData.coordinatorLastName.trim() !== "") ||
      formData.skip
    );
  };
          ...
          <Box className="w-[516px] h-[452px]" sx={style}>
            ...
            <Button
              ...
              sx={{
                ...
                background: canGoToNextPage() ? "#19b394" : "#aaaaaa",
              }}
          ...
          <Box className="w-[516px] " sx={style}>
            ...
            <FormGroup>
              <FormControlLabel
                ...
                label="Deseja pular adição do coordenador?"
              />
            </FormGroup>
            <Button
              ...
              sx={{
                textTransform: "capitalize",
                background: canFinish() ? "#19b394" : "#aaaaaa",
              }}
              className="h-[41px] px-4 py-2 hover:bg-[--primary-dark] mt-6 rounded justify-center items-center gap-3 inline-flex text-white"
            >
              <div className="text-white text-sm font-semibold font-['Source Sans Pro'] leading-[18px]">
