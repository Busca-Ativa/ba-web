...
const UsuariosAdmin = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await api.get('/coordinator/institution/users', {withCredentials: true})
        const dataFromApi = response.data;
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await api.get('/coordinator/institution/users?active=0', {withCredentials: true})
        const dataFromApi = response.data;
        const rows = dataFromApi.data.map( (user: any) => {
          console.log(user)
          return {
        ...
        <>
          <div className="w-[100%] h-[100vh px-[45px] pt-[60px] flex flex-col gap-8 2xl:gap-10">
            <div className="flex justify-between">
              ...
              <Button onClick={handleHideApproval} className="h-[41px] px-4 py-2 bg-[#19b394] hover:bg-[--primary-dark] rounded justify-center items-center gap-3 inline-flex text-white">
                <div className="text-white text-sm font-semibold font-['Source Sans Pro'] leading-[18px]">
                  Voltar
                </div>
              </Button>
            ...
            onDisapprove={ (row: Record<string, string | number>, rowIndex: number) => {
                handleDelete(row, rowIndex)
              }
            }
            onApprove={ (row: Record<string, string | number>, rowIndex: number) => {
